...
REFRESH_SELECTIONS = ('none', 'single-first', 'single-middle', 'single-last',
	'middle-block', 'scattered', 'all-except-current', 'all')


def refresh_selection(pattern, count):
	if count < 8:
		raise ValueError('Refresh requires at least eight rows')
	middle = count // 2
	cursor = {'single-first': 0, 'single-last': count - 1}.get(pattern, middle)
	if pattern == 'none':
		ranges = []
	elif pattern.startswith('single-') and pattern in REFRESH_SELECTIONS:
		ranges = [(cursor, cursor)]
	elif pattern == 'middle-block':
		ranges = [(middle, middle + max(1, count // 10) - 1)]
	elif pattern == 'scattered':
		ranges = [(row, row) for row in sorted({
			index * (count - 1) // (min(100, count) - 1)
			for index in range(min(100, count))})]
	elif pattern == 'all-except-current':
		ranges = [(0, middle - 1), (middle + 1, count - 1)]
	elif pattern == 'all':
		ranges = [(0, count - 1)]
	else:
		raise ValueError('Unknown refresh selection: ' + pattern)
...
def child(directory, label, mode, show_hidden, rows_output=None, interactions=False, output=None, baseline_ref='56e840a', viewport=None, navigation_repetitions=0, refresh_patterns=()):
	...
	def snapshot_commit(model, result):
		...
		if model.get_location() == target:
			...
			if operation:
				if result.listing is not operation.get('previous_listing'):
	...
	with temporary_settings as temporary:
		...
		def exercise():
			...
			try:
				...
				if navigation_repetitions:
					from fman_performancetest.navigation import measure
					state['navigation'] = measure(view, gui, navigation_repetitions)
				if state['errors']:
				...
				if refresh_patterns:
					...
					state['samples'] = []
					def pane_state():
						spans = sorted((item.top(), item.bottom()) for item in view.selectionModel().selection())
						merged = []
						...
						return view.currentIndex().row(), view.verticalScrollBar().value(), merged
					for pattern in refresh_patterns:
						def prepare_refresh():
							model = view.model()
							source = model.sourceModel()
							row, spans = refresh_selection(pattern, model.rowCount())
							...
							return source._displayed, source._visible, source._scan_revision, pane_state()
						previous, visible, revision, before = gui(prepare_refresh)
						memory_before = win32process.GetProcessMemoryInfo(win32api.GetCurrentProcess())
						cpu_start = process_time()
						operation.update(start=perf_counter(), done=Event(), commits=[], previous_listing=previous)
						gui(pane.reload)
						if not operation['done'].wait(90):
							raise TimeoutError(pattern + ' refresh completed paint')
						...
						def check_refresh():
							...
							if pane_state() != before:
								raise RuntimeError(pattern + ' refresh changed marks, cursor or scroll')
						gui(check_refresh)
						state['samples'].append(dict(action_id='refresh.' + pattern,
							paint_ms=operation['paint_ms'], qt_commit_ms=sum(operation['commits']),
							cpu_ms=cpu_ms, selected_count=sum(last - first + 1 for first, last in before[2]),
							cursor_row=before[0], scroll=before[1], state_preserved=True,
							working_set_before_mib=memory_before['WorkingSetSize'] / 1048576,
							working_set_mib=memory_after['WorkingSetSize'] / 1048576,
							peak_working_set_before_mib=memory_before['PeakWorkingSetSize'] / 1048576,
							peak_working_set_mib=memory_after['PeakWorkingSetSize'] / 1048576))
						operation.clear()
					state['peak_working_set_mib'] = memory_after['PeakWorkingSetSize'] / 1048576
					if state['errors']:
						raise RuntimeError('refresh exceptions: ' + repr(state['errors']))
				if interactions:
...
def main():
	...
	for directory in args.directories:
		label = directory.parent.name if directory.name == 'data' else directory.name or 'DriveRoot'
