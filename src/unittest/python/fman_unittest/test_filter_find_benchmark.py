	...
	def test_measure_runs_full_suite_saves_report_and_opens_browser(self):
		with TemporaryDirectory() as directory:
			...
			def run_suite(arguments, *, record_saved):
				self.assertEqual(['--results', str(Path(directory) / 'runs')], arguments)
			...
			with patch.object(report, 'HISTORY', Path(directory)), \
				...
				browser.assert_called_once_with((Path(directory) / 'index.html').as_uri())
			self.assertEqual('<html>report</html>', (Path(directory) / 'index.html').read_text())
			self.assertEqual({'Unreleased'}, set(report.read_history(directory)))
