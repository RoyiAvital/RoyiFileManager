...
def _get_data_directory():
	override = getenv('ROYIFILEMANAGER_USER_SETTINGS')
	if override:
		return abspath(override)
	if getattr(sys, 'frozen', False):
		application_dir = dirname(sys.executable)
	else:
		application_dir = dirname(dirname(dirname(dirname(dirname(__file__)))))
	return join(application_dir, 'UserSettings')
