...
class DevelopmentApplicationContext(ApplicationContext):
	...
	def show_shortcuts(self):
		pane = self.plugin_support.get_panes()[0]
		pane_commands = pane.get_commands()
		application_commands = self.plugin_support.get_application_commands()
		shortcuts = collect_shortcuts(
			self.config.locate('Key Bindings.json'),
			pane_commands | application_commands
		)
		def get_command_title(command):
			if command in pane_commands:
				aliases = pane.get_command_aliases(command)
			elif command in application_commands:
				aliases = self.plugin_support.get_application_command_aliases(command)
			else:
				return command.replace('_', ' ').capitalize()
			return aliases[0] if aliases else command.replace('_', ' ').capitalize()
		ShortcutsDialog(
