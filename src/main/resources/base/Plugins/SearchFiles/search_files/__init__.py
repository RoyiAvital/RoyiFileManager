...
class SearchSession:
	...
	def action(self, name, values):
		...
		if self.root is None:
			self.panel.set_activity_status('File search requires a local folder.')
			return
		try:
		...
		if not runner.start(lambda result: self.completed(generation, result)):
			self.runner = None
			self.enable_form()
			self.panel.set_activity_status('Another file search is still running or stopping.')
...
class SearchFiles(DirectoryPaneCommand):
	...
	def __call__(self):
		...
		if not isinstance(path, str) or not path.startswith('file://'):
			show_status_message('File search requires a local folder.', timeout_secs=3)
			return
		try:
