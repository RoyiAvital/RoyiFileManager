...
`tinycss2`, keeping every observable theme behaviour. Status: Design; awaiting
review. Source of the request: the `tinycss` item in [TODO.md](../TODO.md).
...
Goal: no `tinycss` import anywhere, no new warnings, identical theme output and
identical error reporting for every shipped and test theme.
...
- Introduce `fman.impl.util.css.CSSParseError(ValueError)` carrying `line`,
  ...
  `tinycss.parsing.ParseError`. The `ThemeError` message text is unchanged.
...
authors see the same `Theme.css` behaviour; the only user-visible change is the
absence of the SyntaxWarning. `tinycss` and `tinycss2` were never part of the
plug-in API surface.
...
| `tinycss2` (conda-forge noarch, ≥1.2) | – | required |
...
mention `tinycss` today and needs no change. `tinycss2` is not installed in the
environment yet; the user installs it (or re-creates the environment from the
updated lock) before Implementation Step 2.
...
- `grep -r tinycss src/` returns nothing except `tinycss2` imports in `css.py`.
...
- Invalid themes raise `ThemeError` with the existing
  ...
  1-based positions.
- `environment.yml` and `conda-lock.yml` no longer list `tinycss`; they list
  `tinycss2` and `webencodings`.
